...
class _MongoCore(_BaseCore):
    ...
    def __init__(self, mongetter, hash_func, wait_for_calc_timeout, default_params):
        if "pymongo" not in sys.modules:
            warnings.warn(
                ("Cachier warning: pymongo was not found. " "MongoDB cores will not function.")
            )  # pragma: no cover
        ...
        if _MongoCore._INDEX_NAME not in index_inf:
            func1key1 = IndexModel(keys=[("func", ASCENDING), ("key", ASCENDING)], name=_MongoCore._INDEX_NAME)
    ...
    def get_entry_by_key(self, key):
        res = self.mongo_collection.find_one({"func": _MongoCore._get_func_str(self.func), "key": key})
        if res:
            try:
                entry = {
    ...
    def clear_cache(self):
        self.mongo_collection.delete_many(filter={"func": _MongoCore._get_func_str(self.func)})

    def clear_being_calculated(self):
        self.mongo_collection.update_many(
            filter={"func": _MongoCore._get_func_str(self.func), "being_calculated": True},
            update={"$set": {"being_calculated": False}},
        )
