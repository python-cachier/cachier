...
def extract_requirements(setup_py_path, requirements_txt_path):
    ...
    match = re.search(r"install_requires=\[(.*?)\]", setup_py_contents, re.DOTALL)
    if match:
        # Extract the matched group, split by comma, and strip whitespace and quotes
        requirements = [req.strip().strip("'\"") for req in match.group(1).split(',')]
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python script.py <path_to_setup.py> <path_to_requirements.txt>")
        sys.exit(1)
