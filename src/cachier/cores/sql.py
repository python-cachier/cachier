...
from sqlalchemy import (
    create_engine, Column, String, LargeBinary, DateTime, Boolean, select, update, insert, delete, and_, Index
)
...
from .base import _BaseCore, RecalculationNeeded, _get_func_str
...
class _SQLCore(_BaseCore):
    """
    SQLAlchemy-based core for Cachier, supporting SQLite, PostgreSQL, MySQL, etc.
    """
    def __init__(
        self,
        ...
        super().__init__(hash_func=hash_func, wait_for_calc_timeout=wait_for_calc_timeout)
        ...
        with self._lock, self._Session() as session:
            row = session.execute(
                select(CacheTable).where(
                    and_(CacheTable.function_id == self._func_str, CacheTable.key == key)
                )
            ).scalar_one_or_none()
            if not row:
                return key, None
    ...
    def set_entry(self, key: str, func_res: Any) -> None:
        with self._lock, self._Session() as session:
            ...
            ) if hasattr(insert(CacheTable), 'on_conflict_do_update') else None
    ...
    def clear_cache(self) -> None:
        with self._lock, self._Session() as session:
            session.execute(
                delete(CacheTable).where(CacheTable.function_id == self._func_str)
            )
            session.commit()
    ...
    def clear_being_calculated(self) -> None:
        with self._lock, self._Session() as session:
            session.execute(
                update(CacheTable)
                .where(and_(CacheTable.function_id == self._func_str, CacheTable.processing == True))
                .values(processing=False)
            )
            session.commit() 