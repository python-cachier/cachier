...
def test_pickle_core(reload, separate_files):
    ...
    _takes_2_seconds_decorated = _get_decorated_func(
        _takes_2_seconds, next_time=False, pickle_reload=reload, separate_files=separate_files
    )
...
_BAD_CACHE_FNAME_SEPARATE_FILES = (
    ".tests.test_pickle_core._bad_cache_" f"{hashlib.sha256(pickle.dumps((0.13, 0.02))).hexdigest()}"
)
...
_DEL_CACHE_FNAME_SEPARATE_FILES = (
    ".tests.test_pickle_core._delete_cache_" f"{hashlib.sha256(pickle.dumps((0.13, 0.02))).hexdigest()}"
)
