...
def test_mongo_delete_stale_direct():
    ...
    @cachier.cachier(
        mongetter=_test_mongetter, stale_after=timedelta(seconds=1)
    )
    def test_func(x):
        return x * 2
...
def test_pickle_clear_being_calculated_separate_files():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        @cachier.cachier(
            backend="pickle", cache_dir=temp_dir, separate_files=True
        )
        def test_func(x):
            return x * 2
...
def test_redis_eviction_delete_failure():
    ...
    @cachier.cachier(
        backend="redis", redis_client=client, cache_size_limit="150B"
    )
...
def test_pickle_wait_timeout_check():
    ...
    @cachier.cachier(backend="pickle", wait_for_calc_timeout=0.2)
    def slow_func(x):
